	...
	<div class="page-wrap">
		...
		<main class="form-signin text-center">
			...
			if ( isset($_REQUEST['signup-name']) && isset($_REQUEST['signup-email']) && isset($_REQUEST['signup-password']) ) {
				...
				$local_currency = 'eur';
				$users = new users();
				$users_existe = $users->getData($email);
				if ( !$users_existe ) {
				// Compruebo si existe el usuario
					$user_created = $users->create($name, $lastname, $email, $password, $create_datetime, $local_currency);
					if ($user_created) {
						// si se crea el usuario, creamos la wallet de fondos
						$wallets = new wallets();
						$deposit_local_currency = 'eur';
						$wallet_id = md5( $deposit_local_currency . $email );
						$create_datetime = date("Y-m-d H:i:s");
						$deposit_amount = 0;
						$deposit_result = $wallets->create($wallet_id, $email, $deposit_local_currency, $deposit_amount, $create_datetime);
						echo "<div class='form'>
							<p class='text-success'>Te has registrado correctamente.</p>
							<p class='link'>Puedes acceder <a href='./login.php' class=''>aquí</a>.</p>
							</div>";
					} else {
						echo "<div class='form'>
					...
					echo "<div class='form'>
						<p class='text-danger'>Ya existe un usuario con este email.</p>
						<p class='link'>Puedes acceder <a href='./login.php' class=''>aquí</a>.</p>
						</div>";
				}
			} else {
			...
			<form id="form-signup" action="" method="post">
				...
				<div class="row mb-2 g-2">
					<div class="col-sm">
						<div class="form-floating">
							<input type="password" class="form-control" id="signup-password" name="signup-password" placeholder="Contraseña" required>
